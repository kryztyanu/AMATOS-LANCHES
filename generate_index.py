...
<div class="modal-overlay" id="cartModal">
    <div class="modal-content">
        ...
        <div style="display: flex; gap: 8px; margin-top: 12px;">
            <button class="btn-secondary" style="flex: 1; border: 2px solid var(--red-main); color: var(--red-dark); background: white; border-radius: 10px; font-weight: 800; padding: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px;" onclick="printOrderReceipt()">
                <span>🖨️ IMPRIMIR</span>
            </button>
            <button class="btn-whatsapp" style="flex: 2; margin-top: 0;" onclick="sendWhatsAppOrder()">
...
<script>
    ...
    function printOrderReceipt() {
        ...
        const name = document.getElementById('custName').value.trim() || 'Cliente';
        const address = document.getElementById('custAddress').value.trim() || 'Retirada no Balcão';
        const reference = document.getElementById('custRef').value.trim();
        const orderObs = document.getElementById('custOrderObs').value.trim();
        let payment = document.getElementById('custPayment').value;
        const troco = document.getElementById('custTroco').value.trim();
        if (payment === 'Dinheiro' && troco) {
        ...
        let itemsText = cart.map(item => {
            let sub = item.price ? `R$ ${(item.price * item.quantity).toFixed(2).replace('.', ',')}` : '';
            return `${item.quantity} ${item.name} ${item.selectedOption ? `(${item.selectedOption})` : ''} - ${sub}${item.observation ? `\n   Obs: ${item.observation}` : ''}`;
        }).join('\n');
        ...
        printWindow.document.write(`
            ...
            <head>
                ...
                <style>
                    body { font-family: monospace; font-size: 13px; padding: 15px; margin: 0; }
            ...
            <body>
                <div class="center bold" style="font-size: 16px;">NOVO PEDIDO:</div>
                <div class="line"></div>
                <div><b>Cliente:</b> ${name}</div>
                <div><b>Endereço:</b> ${address}</div>
                ${reference ? `<div><b>Ref:</b> ${reference}</div>` : ''}
                ${orderObs ? `<div><b>Obs:</b> ${orderObs}</div>` : ''}
                <div><b>Pagamento:</b> ${payment}</div>
                <div class="line"></div>
                <div class="bold">ITENS DO PEDIDO:</div><br/>
                <pre style="font-family: monospace; white-space: pre-wrap;">${itemsText}</pre>
                <div class="line"></div>
                <div class="bold" style="font-size: 14px;">TOTAL DO PEDIDO: R$ ${totalPrice.toFixed(2).replace('.', ',')}</div>
                <div class="bold" style="color: #c00; font-size: 12px; margin-top: 4px;">+ TAXA DE ENTREGA: consulte valor da taxa</div>
                <div class="line"></div>
                <div class="center">Obrigado pelo pedido!</div>
                <script>
